	define SNA_FILENAME "test-1s-attr.sna"
...
start	di : ld sp, $-2
	...
	; инициация плеера
	ld a, #42 : ld (chnk_main.CUR_ATTR), a	; цвет, которым будут отображаться чанки
...
fin_outer	push bc
	...
	; change color
	ld a, #47 : ld (chnk_main.CUR_ATTR), a

	halt : halt : halt : halt

	; restore color
	ld a, #42 : ld (chnk_main.CUR_ATTR), a
...
CHNK_START	include "res/parsed-1s-attr.asm"
